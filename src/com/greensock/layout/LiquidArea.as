 ...
 * (or simply use it like a regular Shape by adding it to the display list with addChild(), but the 
 ...
 */	
	public class LiquidArea extends AutoFitArea {
		/** @private **/
		public static const version:Number = 2.04;
		 ...
		 */
		public function LiquidArea(parent:DisplayObjectContainer, x:Number, y:Number, width:Number, height:Number, previewColor:uint=0xFF0000, minWidth:Number=0, minHeight:Number=0, maxWidth:Number=99999999, maxHeight:Number=99999999, autoPinCorners:Boolean=true) {
			...
			if (autoPinCorners) {
				if (_parent.stage) {
					...
					_parent.addEventListener(Event.ADDED_TO_STAGE, this.autoPinCorners, false, 0, true);
				}
			}
		}
		 ...
		 */
		public static function createAround(target:DisplayObject, scaleMode:String=ScaleMode.PROPORTIONAL_INSIDE, hAlign:String=AlignMode.CENTER, vAlign:String=AlignMode.CENTER, crop:Boolean=false, previewColor:uint=0xFF0000, minWidth:Number=0, minHeight:Number=0, maxWidth:Number=99999999, maxHeight:Number=99999999, autoPinCorners:Boolean=true):LiquidArea {
			var bounds:Rectangle = target.getBounds(target.parent);
			var la:LiquidArea = new LiquidArea(target.parent, bounds.x, bounds.y, bounds.width, bounds.height, previewColor, minWidth, minHeight, maxWidth, maxHeight, autoPinCorners);
			la.attach(target, scaleMode, hAlign, vAlign, crop);
			return la;
		}
		...
		protected function autoPinCorners(event:Event=null):void {
			if (event) {
				_parent.removeEventListener(Event.ADDED_TO_STAGE, autoPinCorners);
			}
			_liquidStage = LiquidStage.getByStage(_parent.stage);
			if (_liquidStage && _topLeftPin == null) {
				pinCorners(_liquidStage.TOP_LEFT, _liquidStage.BOTTOM_RIGHT, false, 0, null);
			}
		}
		 ...
		 */
		public function pinCorners(topLeft:PinPoint, bottomRight:PinPoint, reconcile:Boolean=false, tweenDuration:Number=0, tweenVars:Object=null):void {
			if (_topLeftPin != null) {
				_data.destroy(false);
				if (_tweenMode) {
					_tween.kill();
			...
			_data = new LiquidData(_topLeftPin, this, 3, false, 0, null);
